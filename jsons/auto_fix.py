...
def auto_fix_json_string(s):
    # 尝试修复常见的JSON格式错误：将单个反斜线转换为双反斜线
    return s.replace('\\', '\\\\')
...
def check_criteria(shapes, min_instance, min_salient, min_camouflage):
    ...
    return (instance_count > min_instance or salient_count > min_salient or camouflage_count > min_camouflage)
    ...
    with open(error_log_path, 'w') as log_file:
        for filename in tqdm(os.listdir(args.dir)):
            ...
            try:
                ...
                try:
                    with open(file_path, "r") as f:
                        fixed_json = auto_fix_json_string(f.read())
                        labelme_json = json.loads(fixed_json)
                except json.JSONDecodeError as e:
                    log_file.write(f"{filename}: {str(e)}\n")
                    continue  # 继续处理下一个文件

            if check_criteria(labelme_json["shapes"], args.min, args.salient, args.camouflage):
                shutil.copy(file_path, os.path.join(args.out, filename))
